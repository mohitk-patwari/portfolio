            ...
            <motion.div variants={itemVariants} className="mt-16 flex flex-wrap items-center gap-6">
              {metrics.map((metric, index) => (
                <div key={metric.label} className="flex items-center gap-6">
                  <div>
                    <p className="font-display text-2xl text-lemon glow-lemon">{metric.value}</p>
                    <p className="font-mono text-xs uppercase tracking-widest text-butter/50">{metric.label}</p>
                  </div>
                  {index < metrics.length - 1 && <div className="h-10 w-px bg-borderline" />}
                </div>
              ))}
            </motion.div>
          </div>
